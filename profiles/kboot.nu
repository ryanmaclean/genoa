...
def run_step [step: record, dry_run: bool] {
    let is_linux = try { (^uname -s | str trim) == "Linux" } catch { false }
    if $dry_run or not $is_linux {
        return $step
    }
    let result = try {
...
export def kboot_build [manifest: record, dry_run: bool = false] {
    ...
    # Step 18a — write rendered loader.kboot.conf to disk
    let loader_kboot_conf_content = $"# loader.kboot.conf — generated by genoa kboot profile\nhostname=($hostname)\nvfs.root.mountfrom=\"ufs:/dev/vtbd0p3\"\n"
    let step18a = if $dry_run {
        {step: "18a" label: "write_loader_kboot_conf" action: "would-run"
         description: "Write loader.kboot.conf to /mnt/freebsd/boot/loader.kboot.conf"
         output_path: "/mnt/freebsd/boot/loader.kboot.conf"
         preview: ($loader_kboot_conf_content | str substring 0..120)}
    } else {
        try {
            $loader_kboot_conf_content | save --force "/mnt/freebsd/boot/loader.kboot.conf"
            {step: "18a" label: "write_loader_kboot_conf" action: "ran" exit_code: 0
             description: "Wrote loader.kboot.conf to /mnt/freebsd/boot/loader.kboot.conf"
             output_path: "/mnt/freebsd/boot/loader.kboot.conf"}
        } catch { |e|
            {step: "18a" label: "write_loader_kboot_conf" action: "failed" exit_code: 1
             stderr: $e.msg}
        }
    }
    if $step18a.action == "failed" {
        return {action: "build-failed", failed_step: $step18a.label, exit_code: 1, detail: $step18a}
    }

    # Step 18b — write rc.conf to disk
    let rc_conf_content = $"hostname=\"($hostname)\"\nifconfig_vtnet0=\"DHCP\"\nsshd_enable=\"YES\"\n"
    let step18b = if $dry_run {
        {step: "18b" label: "write_rc_conf" action: "would-run"
         description: "Write rc.conf to /mnt/freebsd/etc/rc.conf"
         output_path: "/mnt/freebsd/etc/rc.conf"
         preview: ($rc_conf_content | str substring 0..120)}
    } else {
        try {
            $rc_conf_content | save --force "/mnt/freebsd/etc/rc.conf"
            {step: "18b" label: "write_rc_conf" action: "ran" exit_code: 0
             description: "Wrote rc.conf to /mnt/freebsd/etc/rc.conf"
             output_path: "/mnt/freebsd/etc/rc.conf"}
        } catch { |e|
            {step: "18b" label: "write_rc_conf" action: "failed" exit_code: 1
             stderr: $e.msg}
        }
    }
    if $step18b.action == "failed" {
        return {action: "build-failed", failed_step: $step18b.label, exit_code: 1, detail: $step18b}
    }
    ...
    let plan = {
        ...
        steps: [
            ...
            $step14 $step15 $step16 $step17 $step18_render $step18a $step18b $step19 $step20 $step21
        ]
        receipt_path: ""
        notes: {
            version: "1.0.0"
