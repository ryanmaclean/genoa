...
export def uefi_build [manifest: record, dry_run: bool = false] {
    ...
    # ── step 9d: write_fstab ─────────────────────────────────────────────────
    # Minimal /etc/fstab — FreeBSD boots without it but logs repeated warnings.
    # GPT label "rootfs" matches gpart add -l rootfs in step 4.
    let fstab_content = "# /etc/fstab — generated by genoa\n/dev/gpt/rootfs\t/\tufs\trw\t1\t1\n"
    let step9d = if $dry_run {
        {step: "9d" label: "write_fstab" action: "would-run"
         cmd: "printf '...' > /mnt/rootfs/etc/fstab" description: "Write minimal /etc/fstab (silences boot warnings)"}
    } else {
        try {
            $fstab_content | save --force "/mnt/rootfs/etc/fstab"
            {step: "9d" label: "write_fstab" action: "ran" exit_code: 0 output: "/mnt/rootfs/etc/fstab written"}
        } catch { |e|
            {step: "9d" label: "write_fstab" action: "failed" exit_code: 1 stderr: $e.msg}
        }
    }
    if $step9d.action == "failed" {
